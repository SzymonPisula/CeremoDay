...
function validatePayload(body: unknown):
  | { ok: true; payload: InterviewPayload }
  ...
  if (!isOneOf(ceremony_type, CEREMONY_TYPES))
    return { ok: false, message: `Niepoprawne ceremony_type: ${String(ceremony_type)}. Dozwolone: ${CEREMONY_TYPES.join(", ")}.` };

  if (!isOneOf(guest_count_range, GUEST_RANGES))
    return { ok: false, message: `Niepoprawne guest_count_range: ${String(guest_count_range)}. Dozwolone: ${GUEST_RANGES.join(", ")}.` };

  if (!isOneOf(guest_list_status, GUEST_LIST_STATUSES))
    return { ok: false, message: `Niepoprawne guest_list_status: ${String(guest_list_status)}. Dozwolone: ${GUEST_LIST_STATUSES.join(", ")}.` };

  if (!isOneOf(music_provider_choice, MUSIC_CHOICES))
    return { ok: false, message: `Niepoprawne music_provider_choice: ${String(music_provider_choice)}. Dozwolone: ${MUSIC_CHOICES.join(", ")}.` };

  if (!isOneOf(venue_choice, VENUE_CHOICES))
    return { ok: false, message: `Niepoprawne venue_choice: ${String(venue_choice)}. Dozwolone: ${VENUE_CHOICES.join(", ")}.` };
...
function mapCeremonyToModel(v: CeremonyType): ModelCeremonyType {
  switch (v) {
    ...
    case "reception_only": return "RECEPTION_ONLY";
  }
}
...
function mapNotifToModel(v: NotificationFrequency): ModelNotificationFrequency {
  switch (v) {
    ...
    case "every_3_days": return "EVERY_3_DAYS";
    case "weekly": return "WEEKLY";
    case "only_critical": return "ONLY_CRITICAL";
  }
}
...
function mapMusicToModel(v: MusicProviderChoice): ModelMusicProviderChoice {
  return v; // DJ/BAND są takie same w modelu
}
...
function toIsoMaybe(d?: Date | string | null): string | undefined {
  ...
  if (typeof d === "string") {
    // jeśli sequelize zwróci string – oddajemy string
    return d;
  }
  return undefined;
}
